...
- **v1.0.6：**
  - **支援多螢幕智慧擷取**：修復跨螢幕時無法偵測視窗的問題，現在滑鼠在任一螢幕皆可精準擷取。
  - **修正更新提示功能**：修復「複製更新指令」按鈕失效的問題，點擊後可正確複製指令至剪貼簿。
  
- **v1.0.5**:
  - **優化視窗初始大小**：截圖自動適應螢幕解析度（最高 80%），防止視窗過大超出螢幕。
  - **支援視窗比例縮放**：視窗現在可以自由調整大小，且圖片會自動保持比例不變形。
  
- **v1.0.4**：
  - **新增視窗擷取快捷鍵**：新增 **Cmd + Shift + W** 快速進入智慧視窗偵測模式。
  - **全新底部工具列**：釘選視窗改用底部固定選單，操作更直覺且不遮擋畫面。
