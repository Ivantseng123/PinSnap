    ...
    func triggerInteractiveCapture() {
            DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                ...
                do {
                    ...
                    print("互動截圖啟動失敗: \(error)")
                }
            }
        }
    
    func createNewPinWindow(with image: NSImage) {
...
var onCopyCommand: (() -> Void)?
    ...
    override func sendEvent(_ event: NSEvent) {
            if event.type == .leftMouseDown {
                ...
                if hitView is DrawingOverlayView {
                    super.sendEvent(event)
                    return
                }
...
class PinnedImageWindowController: NSWindowController {
    ...
    private func setupUI(with image: NSImage) {
            ...
            NSLayoutConstraint.activate([
                ...
                self.penBtn = createFloatingButton(symbolName: "pencil", action: #selector(toggleDrawingMode(_:)), isToggle: true)
                controlsContainer.addSubview(self.penBtn)
                ...
                NSLayoutConstraint.activate([
                    drawingToolsStack.centerYAnchor.constraint(equalTo: controlsContainer.centerYAnchor),
                    drawingToolsStack.leadingAnchor.constraint(equalTo: controlsContainer.leadingAnchor, constant: 16),
                    drawingToolsStack.heightAnchor.constraint(equalToConstant: 28),
                    
                    sliderContainer.centerYAnchor.constraint(equalTo: controlsContainer.centerYAnchor),
                    sliderContainer.trailingAnchor.constraint(equalTo: controlsContainer.trailingAnchor, constant: -16),
                    ...
                    saveBtn.centerYAnchor.constraint(equalTo: controlsContainer.centerYAnchor),
                    saveBtn.trailingAnchor.constraint(equalTo: sliderContainer.leadingAnchor, constant: -16),
                    
                    copyBtn.centerYAnchor.constraint(equalTo: controlsContainer.centerYAnchor),
                    copyBtn.trailingAnchor.constraint(equalTo: saveBtn.leadingAnchor, constant: -8),
                    
                    penBtn.centerYAnchor.constraint(equalTo: controlsContainer.centerYAnchor),
        ...
        private func setupLiveTextOCR(with image: NSImage) {
            if #available(macOS 13.0, *) {
                ...
                imageView.superview?.addSubview(overlayView, positioned: .above, relativeTo: imageView)
    ...
    @objc private func toggleDrawingMode(_ sender: NSButton) {
            ...
            updatePenButtonAppearance()
        }
    
        private func updatePenButtonAppearance() {
            let isDrawing = drawingOverlay.isDrawingMode
            ...
            NSAnimationContext.runAnimationGroup { context in
                ...
                if isDrawing {
                    let strokeColor = drawingOverlay.strokeColor
                    let displayColor = strokeColor == .black ? NSColor.white : strokeColor
                    
                    self.penBtn.contentTintColor = displayColor
                    self.penBtn.layer?.borderColor = displayColor.withAlphaComponent(0.8).cgColor
                    self.penBtn.layer?.backgroundColor = displayColor.withAlphaComponent(0.15).cgColor
                } else {
                    self.penBtn.contentTintColor = .white
                    self.penBtn.layer?.borderColor = NSColor.white.withAlphaComponent(0.3).cgColor
                    self.penBtn.layer?.backgroundColor = NSColor.black.withAlphaComponent(0.65).cgColor
                }
            }
        }
    ...
    @objc private func setColorBlack() {
            drawingOverlay.strokeColor = .black
            updatePenButtonAppearance()
        }
        
        @objc private func setColorRed() {
            drawingOverlay.strokeColor = .systemRed
            updatePenButtonAppearance()
        }
        
        @objc private func setColorBlue() {
            drawingOverlay.strokeColor = .systemBlue
            updatePenButtonAppearance()
        }
    ...
    @objc private func pickColor() {
            ...
            sampler.show { [weak self] selectedColor in
                if let color = selectedColor {
                    ...
                    self?.updatePenButtonAppearance()
                }
            }
        }
